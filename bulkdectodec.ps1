...
$FileMatches = @()
$FileMatches += Select-String -Path $Path -Pattern "(\d+)" -AllMatches | Select-String -Pattern "name","version","\/\/","table","tcam","LUT","BASE" -NotMatch | Group-Object -Property "Path" 
...
$FileMatches | ForEach-Object -Process {       
    ...
    $_.Group | ForEach-Object -Process {
        ...
        $_.Matches | ForEach-Object -Process {
            ...
            $Content[$Index] = $Content[$Index].Insert($_.Index + $Offset,$dec)

            $Offset += ($dec.Length - $_.value.length)
        }
    }
